...
public class JsonSyslogParserTest {
	...
	public void setUp() throws Exception {
		...
		parser.setTimestampRegex("(\\d{4}-\\d{1,2}-\\d{1,2}T\\d{1,2}:\\d{1,2}:\\d{1,2}\\.\\d{1,6})\\s");
		parser.setInputDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSS");
		parser.setOutputDateFormat("MMM dd yyyy HH:mm:ss.SSS");
		parser.initialize();
	}

	@Test
	public void parse() throws Exception {
		parser.setInputStream(new FileInputStream("./src/test/resources/auditd.json"));

		BufferedReader reader = null;
		try {
			reader = new BufferedReader(new FileReader("./src/test/resources/auditd.json"));
			while (reader.ready()) {
				String line = reader.readLine();
				parser.setInputStream(new ByteArrayInputStream(line.getBytes("UTF8")));
				Map<String, String> fields = parser.parseLogFields();
				assertEquals("audispd", fields.get("APP_NAME"));
				assertEquals("secap01", fields.get("HOSTNAME"));

				if (fields.containsKey("PROCID")) {
					assertEquals("1234", fields.get("PROCID"));
				}
			}
		} finally {
